...
class file_backend_req : public request
{
  ...
  public:
    file_backend_req(enum lsvd_op op_, const char *name_, iovec *iov,
                     int iovcnt, size_t offset_, file_backend *be_)
        : _iovs(iov, iovcnt)
    {
        op = op_;
        offset = offset_;
...
void file_backend::worker(thread_pool<file_backend_req *> *p)
{
    ...
    while (p->running) {
        ...
        if (__lsvd_dbg_be_delay) {
            std::uniform_int_distribution<int> uni(0, __lsvd_dbg_be_delay_ms *
                                                          1000);
