...
void write_cache_impl::write_checkpoint(void)
{
    ...
    if (map.size() > 0)
        for (auto it = map.begin(); it != map.end(); it++)
            jme[n_extents++] = (j_map_extent){
                (uint64_t)it->s.base, (uint64_t)it->s.len, (uint32_t)it->s.ptr};
    ...
    iovec iov[] = {{e_buf, map_pages * 4096UL}, {l_buf, len_pages * 4096UL}};
                                   ...
                                   lsvd_config *cfg_)
{
    ...
    misc_threads->pool.push(
        std::thread(&write_cache_impl::flush_thread, this, misc_threads));
}
...
void write_cache_impl::send_writes(std::unique_lock<std::mutex> &lk)
{
    ...
    auto req =
        new wcache_write_req(&work, pages, page, n_pad - 1, pad, prev, this);
    for (auto w : work) {
        auto [iov, iovcnt] = w->iov->c_iov();
...
write_cache_work *write_cache_impl::writev(request *req, sector_t lba,
                                           smartiov *iov)
{
    std::unique_lock lk(m);
