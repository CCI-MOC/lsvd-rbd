...
class nvme_uring : public nvme
{
  ...
  public:
    ...
    std::unique_ptr<request> make_write_request(smartiov *iov, size_t offset)
    {
        assert(offset != 0);
        return std::make_unique<nvme_uring_request>(iov, offset, OP_WRITE,
                                                    this);
    }

    std::unique_ptr<request> make_write_request(char *buf, size_t len,
                                                size_t offset)
    {
        assert(offset != 0);
        smartiov iov(buf, len);
        return std::make_unique<nvme_uring_request>(&iov, offset, OP_WRITE,
                                                    this);
    }

    std::unique_ptr<request> make_read_request(smartiov *iov, size_t offset)
    {
        return std::make_unique<nvme_uring_request>(iov, offset, OP_READ, this);
    }

    std::unique_ptr<request> make_read_request(char *buf, size_t len,
                                               size_t offset)
    {
        smartiov iov(buf, len);
        return std::make_unique<nvme_uring_request>(&iov, offset, OP_READ,
                                                    this);
    }

    void uring_completion_worker()
    {
        pthread_setname_np(pthread_self(), "uring_cqe_worker");
    ...
    class nvme_uring_request : public request
    {
      ...
      public:
        nvme_uring_request(smartiov *iovs, size_t offset, lsvd_op op,
                           nvme_uring *nvmu)
            : nvmu_(nvmu), op_(op), iovs_(iovs->data(), iovs->size()),
              offset_(offset)
        {
        }
...
std::unique_ptr<nvme> make_nvme_uring(int fd, const char *name)
{
    return std::make_unique<nvme_uring>(fd, name);
}
