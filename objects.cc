...
{
    return sizeof(obj_hdr) + sizeof(obj_data_hdr) +
...
size_t make_data_hdr(char *hdr, size_t bytes, uint64_t cache_seq,
                     std::vector<data_map> *entries, uint32_t seq, uuid_t *uuid)
{
    auto h = (obj_hdr *)hdr;
    auto dh = (obj_data_hdr *)(h + 1);
    uint32_t o1 = sizeof(*h) + sizeof(*dh), o2 = o1,
             l2 = entries->size() * sizeof(data_map), hdr_bytes = o2 + l2;
