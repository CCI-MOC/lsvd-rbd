...
static std::pair<std::string, std::string> split_string(std::string s,
                                                        std::string delim)
{
    auto i = s.find(delim);
...
void aio_wrapper_done(union sigval siv)
{
    ...
    // aio_op(w->aio.aio_lio_opcode), rv, err); printf("%p done rv=%d err=%d\n",
    // w, rv, err);
    w->cb(w->ptr);
    ios_done++;
    delete w;
...
struct rbd_ops {
    int (*aio_readv)(rbd_image_t image, const iovec *iov, int iovcnt,
                     uint64_t off, rbd_completion_t c);
    int (*aio_writev)(rbd_image_t image, const struct iovec *iov, int iovcnt,
                      uint64_t off, rbd_completion_t c);
    int (*aio_read)(rbd_image_t image, uint64_t offset, size_t len, char *buf,
                    rbd_completion_t c);
    int (*aio_write)(rbd_image_t image, uint64_t off, size_t len,
...
static int emulate_aio_read(rbd_image_t image, uint64_t offset, size_t len,
                            char *buf, rbd_completion_t c)
{
...
static int emulate_aio_write(rbd_image_t image, uint64_t off, size_t len,
                             const char *buf, rbd_completion_t c)
{
    iovec iov = {(void *)buf, len};
...
void rbd_call_wrapped(rbd_completion_t c, void *ptr) { call_wrapped(ptr); }

/* note that rbd_aio_read handles aligned bounce buffers for us
 */
extern "C" ssize_t rbd_read(rbd_image_t image, uint64_t off, size_t len,
                            char *buf)
{
    rbd_completion_t c;
    std::mutex m;
...
extern "C" ssize_t rbd_write(rbd_image_t image, uint64_t off, size_t len,
                             const char *buf)
{
    rbd_completion_t c;
    std::mutex m;
...
static rbd_ops ops_1 = {.aio_readv = NULL,
...
/* implementation 2
 */
static int aio_write_async1(rbd_image_t image, uint64_t off, size_t len,
                            const char *buf, rbd_completion_t c)
{
    fake_rbd_image *fri = (fake_rbd_image *)image;
    auto p = (lsvd_completion *)c;
    ...
    auto aio = new aio_wrapper(fri->fd, (char *)buf, len, nvme_offset,
                               call_wrapped, closure);
    return aio_write(&aio->aio);
}

static rbd_ops ops_2 = {.aio_readv = NULL,
                        .aio_writev = NULL,
                        .aio_read = emulate_aio_read,
...
static int aio_write_async3(rbd_image_t image, uint64_t off, size_t len,
                            const char *buf, rbd_completion_t c)
{
    fake_rbd_image *fri = (fake_rbd_image *)image;
    auto p = (lsvd_completion *)c;
...
static rbd_ops ops_3 = {.aio_readv = NULL,
                        .aio_writev = NULL,
                        .aio_read = emulate_aio_read,
...
int io_queue_run2(io_context_t ctx, struct timespec *timeout)
{
     ...
     * Loop until we have read all the available events and called the
     * callbacks.
     */
    do {
        int i;
...
void async4_close(fake_rbd_image *fri) { io_queue_release(ioctx); }
...
static int aio_write_async4(rbd_image_t image, uint64_t off, size_t len,
                            const char *buf, rbd_completion_t c)
{
    fake_rbd_image *fri = (fake_rbd_image *)image;
    auto p = (lsvd_completion *)c;
...
static int aio_write_async4_2(rbd_image_t image, uint64_t off, size_t len,
                              const char *buf, rbd_completion_t c)
{
    fake_rbd_image *fri = (fake_rbd_image *)image;
    auto p = (lsvd_completion *)c;
...
static rbd_ops ops_4 = {.aio_readv = NULL,
                        .aio_writev = NULL,
                        .aio_read = emulate_aio_read,
...
static int aio_write_async5(rbd_image_t image, uint64_t off, size_t len,
                            const char *buf, rbd_completion_t c)
{
    fake_rbd_image *fri = (fake_rbd_image *)image;
    auto p = (lsvd_completion *)c;
...
static rbd_ops ops_5 = {.aio_readv = NULL,
                        .aio_writev = NULL,
                        .aio_read = emulate_aio_read,
...
static int aio_write_async6(rbd_image_t image, uint64_t off, size_t len,
                            const char *buf, rbd_completion_t c)
{
    fake_rbd_image *fri = (fake_rbd_image *)image;
    auto p = (lsvd_completion *)c;
...
static rbd_ops ops_6 = {.aio_readv = NULL,
                        .aio_writev = NULL,
                        .aio_read = emulate_aio_read,
