...
struct sector_ptr {
  ...
  public:
    ...
    bool operator<(const sector_ptr other) const { return buf < other.buf; }
    bool operator>(const sector_ptr other) const { return buf > other.buf; }
    bool operator==(const sector_ptr other) const { return buf == other.buf; }
    sector_ptr operator+(int val)
    {
    ...
    int operator-(const sector_ptr val) { return (buf - val.buf) / 512; }
};
...
//
template <class T, class T_in, class T_out> struct _extent {
    T s;
  ...
  public:
    ...
    _extent(T_in _base) { s.base = _base; }
    _extent() {}
    ...
    bool operator<(const _extent &other) const { return s.base < other.s.base; }
    ...
    void access(bool val) { s.a = (val ? 1 : 0); }
    void dirty(bool val) { s.d = (val ? 1 : 0); }
    bool a(void) { return s.a != 0; }
    bool d(void) { return s.d != 0; }
};
...
//
template <class T, class T_in, class T_out, int load = 256> struct extmap {
  ...
  public:
    ...
    iterator lower_bound(T_in base)
    {
        ...
        T _key(base);
        auto list_iter =
            std::lower_bound(lists[i]->begin(), lists[i]->end(), _key);
    ...
    iterator _erase(iterator it)
    {
        ...
        if (lists[it.i]->size() > _load / 2)
            ...
            lists[prev]->insert(lists[prev]->end(), lists[pos]->begin(),
                                lists[pos]->end());
    ...
    //
    iterator lookup(T_in base) { return lower_bound(base); }
