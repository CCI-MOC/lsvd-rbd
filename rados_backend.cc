...
/* gross - I switched to creating the names in-place because perf said
 * it was actually taking a lot of time in string functions. I think...
 */
...
ssize_t rados_backend::read_object(const char *name, char *buf,
				   size_t len, size_t offset) {
    return rados_read(io_ctx, name, buf, len, offset);
}

ssize_t rados_backend::read_numbered_object(int seq, char *buf,
					    size_t len, size_t offset) {
    //auto name = std::string(prefix) + "." + hex(seq);
    char name[128];
    sprintf(name, "%s.%08x", prefix, seq);
...
ssize_t rados_backend::read_numbered_objectv(int seq, iovec *iov,
...
int rados_backend::aio_read_num_object(int seq, char *buf,
				       size_t len, size_t offset,
