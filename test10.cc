...
int n_sectors(void)
{
    struct {
        ...
        {0.5, 8}, {0.25, 32}, {0.125, 64}, {0.0625, 128}, {1.0, 4096}};
...
void init_random(void)
{
    ...
    for (long *p = (long *)rnd_data, *end = (long *)(rnd_data + bytes); p < end;
         p++)
        *p = rng();
}
...
std::string get_cache_filename(struct cfg *cfg)
{
    ...
    auto uuid_name =
        std::string(cfg->cache_dir) + "/" + std::string(uuid_str) + ".cache";

    auto stem = fs::path(cfg->obj_prefix).filename();
    auto obj_name =
        std::string(cfg->cache_dir) + "/" + std::string(stem) + ".cache";
...
void lose_writes(struct cfg *cfg)
{
    ...
    for (int i = base; i < limit;) {
        ...
        if (seq == 0)
            seq = hdr->seq - 1;
        if (hdr->magic != LSVD_MAGIC || hdr->type != LSVD_J_DATA ||
            hdr->seq != seq + 1)
            break;
        seq++;
...
void save_wcache_state(struct cfg *cfg)
{
    ...
    int fd = open(cache.c_str(), O_RDONLY);
    pread(fd, wcache_state_buf, 4096,
          1 * 4096); // assume write super is block 1
...
void restore_wcache_state(struct cfg *cfg)
{
    ...
    pwrite(fd, wcache_state_buf, 4096,
           1 * 4096); // assume write super is block 1
    close(fd);
#endif
}
...
void run_test(struct cfg *cfg)
{
	...
	if (p_log)
	    *p_log = 0;
        ...
        int i = 0;
        for (int sector = 0; sector < cfg->image_sectors;
             sector += bufsize / 512) {
            ...
            for (int j = 0; j < bufsize; j += 512) {
                ...
                image_info[sector + j / 512] =
                    (triple){*(int *)p, *(int *)(p + 4), crc};
