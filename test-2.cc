...
void write_super(std::string &name, int ckpt, int next_obj)
{
    ...
    super_hdr sh = {(uint64_t)size,
                    ...
                    (uint32_t)ckpt_len,
                    0,
                    0, // clone bases: offset, len
                    0,
                    0}; // snapshots: offset, len
    char buf[4096], *p = buf;
...
void read_ckpt(const char *img, obj_hdr *hdr, obj_ckpt_hdr *ckpt_hdr,
               std::vector<int> &ckpts, std::vector<x_mapentry> &entries,
...
TEST_CASE("Checkpoint")
{
    ...
    std::vector<x_mapentry> test_entries{
        {0, 8, 1, 1}, {8, 8, 1, 17}, {16, 8, 1, 9}};
...
TEST_CASE("More object/checkpoint tests")
{
    ...
    std::vector<x_mapentry> entries;
    std::vector<ckpt_obj> objs;
    read_ckpt((img + "." + hex(n)).c_str(), &hdr, &ckpt_hdr, ckpts, entries,
              objs);
    CHECK(objs.size() == 2);
    CHECK(objs[0].seq == 1);
    CHECK(objs[1].seq == 3);
