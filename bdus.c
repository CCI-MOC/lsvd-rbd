...
extern int rbd_write(rbd_image_t image, uint64_t off, size_t len,
                     const char *buf);
extern int rbd_flush(rbd_image_t image);

int do_init(struct bdus_ctx *ctx) { return 0; }
...
int do_write(const char *buffer, uint64_t offset, uint32_t size,
             struct bdus_ctx *ctx)
{
    rbd_image_t img = ctx->private_data;
 ...
 */
static const struct bdus_ops device_ops = {.initialize = do_init,
...
static struct bdus_attrs device_attrs = {.logical_block_size = 512,
